<script lang="ts">
  ...
  async function trade() {
    ...
    try {
      ...
      if (!bundlerInitialized && useSmartAccount) {
        // Use a proper bundler wallet (in production, this should be from secure key management)
        const bundlerPrivateKey = '0x' + '1'.repeat(64) // Demo key - replace with proper key management
  ...
  async function tradeDirectly() {
    ...
    if (tokenFrom.address && txData.to) {
      ...
      await approveAllowance(approveArgs)
    ...
    const txArgs = {
      ...
      // Add required fields for Viem v2+
      nonce: undefined, // Let wallet handle nonce
      maxFeePerGas: undefined, // Use gasPrice for legacy transactions
      maxPriorityFeePerGas: undefined, // Use gasPrice for legacy transactions
      kzg: undefined // Required field for blob transactions
    } as const
